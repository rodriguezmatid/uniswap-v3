...
for i in range(start_index, end_index, increment):
    ...
    event = contract.events.Mint().getLogs(fromBlock=i, toBlock=i+increment)

    for event in event:
        amount = event['args']['amount']
        token0 = event['args']['amount0']
        token1 = event['args']['amount1']
...
# Create a DataFrame from the mint events data
df = pd.DataFrame(pools_data)

# Save the DataFrame to a CSV file
csv_filename = 'events_mint.csv'
